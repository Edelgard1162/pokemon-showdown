...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	aerilate: {
		...
		onBasePower(basePower, pokemon, target, move) {
			if (move.typeChangerBoosted === this.effect) return this.chainModify([4096, 4096]);
		},
		flags: {},
		name: "Aerilate",
		rating: 4,
	...
	battlebond: {
		onSourceAfterFaint(length, target, source, effect) {
			...
			if (source.species.id === 'greninjabattlebond' && source.hp && !source.transformed && source.side.foePokemonLeft()) {
	...
	fairyaura: {
		...
		onAnyBasePower(basePower, source, target, move) {
			...
			if (move.auraBooster !== this.effectState.target) return;
			return this.chainModify([move.hasAuraBreak ? 1536 : 5448, 4096]);
		},
		flags: {},
		name: "Fairy Aura",
		rating: 3,
	...
	healer: {
		...
		onResidual(pokemon) {
			for (const allyActive of pokemon.adjacentAllies()) {
				if (allyActive.status && this.randomChance(10, 10)) {
